...
$147$.  The upper bound is established by all-size identities.  The only
exhaustive steps are the finite exact coefficient collection in the pointwise
quotient and the finite lower-bound separation; neither substitutes for an
equidistribution proof.  Figure~\ref{fig:classification-flow} summarizes the
two levels of the argument.
...
\node[font=\small\bfseries,text=black!70] at (0,1.20)
  {(b) Distribution-class classification audit};
...
diagram-level preprocessing.  Panel (b) gives the distribution-classification
audit; its arrows represent, from left to right, pointwise equality,
...
The appendices give the coordinate certificates and full class membership.
...
\qquad j\in E\Rightarrow\phi(j+1)=\phi(j)+1.
\]
...
numerator is unique.  Applying this coefficientwise reduction to the $644$
signatures
gives $523$ distinct canonical forms.  Equal forms have identical tensors for
every $m$, and distinct forms differ as formal series; the independence of
$\mathcal B^{\otimes m}$ then proves the assertion.
\end{proof}
...
\]
Hence the two signatures are not equidistributed.  They occur in Classes~3
and~4, respectively, of Appendix~\ref{app:membership}.
\end{remark}
...
The complete list of uniform coordinate permutations used in the
classification, together with their orbit-connection certificates, is
recorded in Appendix~\ref{app:coordinate-certificates}.

After the kernel-degree reduction, 172 multiple-support pointwise--$rc$ orbits
remain.  Of these, 84 already form rows containing a single pointwise--$rc$
orbit in the final table, while the other 88 lie in 34 rows containing several
pointwise--$rc$ orbits.  The coordinate certificates reduce these 88 orbits to
52 components.  The resulting upper bound is therefore
\[
28+84+52=164.
\]
...
These are exactly the four rows marked $N$ in Appendix~\ref{app:membership}.
...
These are exactly the three rows marked $C$ in
Appendix~\ref{app:membership}.
...
The proof mechanisms used for the individual final distribution classes are
recorded by the proof codes in Appendix~\ref{app:membership}.
...
\begin{proof}
For the upper bound, Theorem~\ref{thm:active} reduces the four occurring
underlying permutations to $1{,}600$ effective shadings, and
Theorem~\ref{thm:signature} maps them onto 644 signatures.  The complete table
in Appendix~\ref{app:membership} partitions these signatures into 146 rows.
The proof code of each row refers to an all-size identity proved in
Sections~\ref{sec:signature} and~\ref{sec:mechanisms}; hence all members of a
row are equidistributed.  This proves that there are at most 146 nonzero
distribution classes.

For the lower bound, choose one representative signature from each row.  For
every pair of distinct rows, exact enumeration finds a size $n\le7$ at which
their distribution polynomials differ.  The accompanying finite certificate
records the smallest separating size and the two exact integer polynomials.
A difference at one size precludes equidistribution, so no two rows can merge.
There are therefore at least 146 nonzero distribution classes.  Every mesh
pattern with layered underlying permutation occurs in its underlying
...
\section{Layer-coordinate certificates}
\label{app:coordinate-certificates}

All coordinate certificates needed here are drawn from the following eleven
...
\]
Every local orbit joined in Table~\ref{tab:coordinate-certificates} consists
of signatures with three supporting coordinates.  Their statistics vanish
when $m<3$, and for these two sizes we take every $U_i$ to be the identity.
The displayed coordinate words apply for $m\ge3$.  Each $U_i$ is a family,
uniform in $m$, satisfying the hypothesis of Theorem~\ref{thm:coordinate};
the $U_i$ are proof certificates, not new classification objects.
\[
...
Table~\ref{tab:coordinate-certificates} is the complete audit for the
layer-coordinate step $200\to164$ in Section~\ref{sec:coordinate}: its
connections reduce the $88$ relevant pointwise--$rc$ orbits to $52$
components, so, together with the $28$ unique-support classes and $84$
singleton multiple-support orbits, $28+84+52=164$.  The first column gives
final class numbers from Table~\ref{tab:detailed-classes}, not global
pointwise--$rc$ orbit labels.  Comma-separated numbers are distinct classes
that share the same within-class connection pattern; they are not identified
with one another.  Within each named class, $1,2,\ldots$ are local labels for
its pointwise--$rc$ orbits.  Thus $a$--$b:U_i$ means that $U_i$ identifies
local orbits $a$ and $b$ separately inside every class named in that row; for
example, $U_1$ identifies local orbits $1$ and $2$ separately in each of
Classes $3$, $11$, and $20$.  The correspondence is explicit in the
supplementary file \texttt{equidistribution\_audit.json}: within each
distribution class, local orbit $a$ is the entry with
\texttt{local\_label} equal to $a$ in the \texttt{local\_orbits} field; that
entry lists its pointwise IDs and member signatures.  For every entry
$a$--$b:U_i$, the supplementary file
\texttt{coordinate\_witnesses.json} records an explicit source signature in
orbit $a$, an explicit target signature in orbit $b$, and the direction of
the identity.

\begin{table}[H]
\centering
\small
\caption{Audit of the layer-coordinate identifications used in the reduction
from $200$ to $164$ coordinate classes.}
\label{tab:coordinate-certificates}
\vspace{4pt}
\begin{tabular}{@{}c l@{}}
\toprule
\shortstack{final class numbers\\from Table~\ref{tab:detailed-classes}}&
within-class pointwise--$rc$ orbit connections\\ \midrule
$3,11,20$&$1$--$2:U_1$\\
$12,22,43$&$1$--$2:U_2$, $1$--$3:U_3$, $2$--$3:U_4$\\
$13,21,23,44$&$1$--$2:U_8$\\
$35$&$1$--$2:U_5$\\
$37,38,65,90,102$&$1$--$2:U_6$\\
$42$&$1$--$2:U_5$, $1$--$3:U_1$, $2$--$3:U_7$\\
$46$&$1$--$2:U_6$, $1$--$3:U_{11}$, $2$--$3:U_7$\\
$48,63,67,92$&$1$--$2:U_9$\\
$53,55$&$1$--$2:U_7$\\
$59,87$&$1$--$2:U_3$, $1$--$3:U_2$, $2$--$3:U_{10}$\\
$79,81$&$1$--$2:U_1$\\
$80$&$1$--$2:U_{11}$\\
\bottomrule
\end{tabular}
\end{table}
\section{Complete signature-membership table}\label{app:membership}

In the table below, a block word such as $111$ denotes $r=(1,1,1)$, a word
such as $012$ denotes $z=(0,1,2)$, and a word such as $023$ denotes
$E=\{0,2,3\}$.  The proof column uses the legend in
Table~\ref{tab:proof-codes}.  Every one of the 644 signatures occurs in
exactly one row.  The row number is the final distribution-class label used
in Table~\ref{tab:coordinate-certificates}.

\paragraph{How to use Table~\ref{tab:detailed-classes}.}
The table records all $644$ signatures, not merely one representative from
each class and not the $393{,}216$ raw mesh diagrams individually.  Given a
pattern $p=(\tau,R)$, first note that $\tau\in\{231,312\}$ places it in
the zero class.  Otherwise the four possible block words are
$123\leftrightarrow111$, $132\leftrightarrow12$,
$213\leftrightarrow21$, and $321\leftrightarrow3$.  Count the shaded active
cells in each internal channel to obtain $z$, and record the shaded external
cells as $E$.  The row containing $(r;z;E)$ is the final distribution class
of $p$, and every other signature in that row is equidistributed with it.

Conversely, let $\Sigma=(r_1,\ldots,r_s;z_1,\ldots,z_s;E)$ be a signature
in that row.  Recover its active shadings by choosing $z_j$ of the $r_j+1$
cells in the $j$th internal channel and shading exactly the external cells
indexed by $E$; every inactive cell may then be shaded independently.  Hence
the number of raw diagrams represented by $\Sigma$ is
\[
2^{\,16-(3+2s+1)}
\prod_{j=1}^s\binom{r_j+1}{z_j}.
\]
Applying this expansion to every signature in the row lists all raw mesh
patterns in the class.

For example, let
\[
p=\pattern{scale=0.8}{3}{1/1,2/3,3/2}{0/0,1/1,1/2,3/1}.
\]
The signature of $p$ is $(12;01;\{0,1\})$, so
Table~\ref{tab:detailed-classes} places $p$ in Class~$19$.

Table~\ref{tab:proof-codes} is the proof-code legend for the proof column of
the final classification table in Appendix~\ref{app:membership}.  The codes
$A,B,R,L$ refer to the reductions already established, whereas $Q,N,C,M$
refer to the distributional bridges used in Section~\ref{sec:bridges}.  They
identify the proof mechanism for a distribution class; they
do not define an additional classification.

\begin{table}[H]
\centering
\small
\caption{Proof-code legend for the final distribution-class table in
Appendix~\ref{app:membership}.}
\label{tab:proof-codes}
\begin{tabular}{@{}c p{10.6cm}@{}}
\toprule
code&meaning\\ \midrule
$A$&kernel-degree theorem with no free external gap\\
$B$&kernel-degree theorem with exactly one free external gap\\
$R$&pointwise equality, possibly followed by reverse-complement within one
pointwise--$rc$ orbit\\
$L$&layer-coordinate relabelling from Theorem~\ref{thm:coordinate}\\
$Q$&uniform cut-shift bridge from Proposition~\ref{prop:uniform-cut-shift},
or the exceptional Class~79 rational identity from
Proposition~\ref{prop:class79}\\
$N$&non-overlapping contraction composition bijection from
Proposition~\ref{prop:unified-contraction}\\
$C$&separator-word complementation composition bijection\\
$M$&MacMahon's major-index--inversion equidistribution\\
\bottomrule
\end{tabular}
\end{table}

Combined codes, such as $LQ$ or $LCQ$, mean that the corresponding row of
Appendix~\ref{app:membership} is connected by a chain of the indicated
bijections.  These codes are all-size proof certificates for the final
classification table rather than additional classification invariants.

\input{equidistribution_classes_detailed.tex}

\paragraph{Finite audit.}
The accompanying exact data record the $523$ pointwise classes, the local
orbit mapping and explicit witnesses used in
Table~\ref{tab:coordinate-certificates}, and a separating distribution
polynomial for every pair of final rows.  The data, audit scripts, and
pattern-to-class lookup tool are available in the
\href{https://github.com/HAO-Phd/length-three-layered-mesh-patterns}{public
GitHub repository}.
They are supplied for reproducibility only; all structural reductions and
all-size equidistribution identities are proved in the text.
