# Classification lookup guide

`lookup_mesh_class.py` is a deterministic reader for the 146-class data in
`equidistribution_audit.json`. It requires only Python 3.10 or later and the
standard library.
...
The result reports the active signature, its distribution class, shaded
inactive cells that do not affect the statistic, every other signature in the
same class, and the number of raw mesh diagrams represented by the signature
and by the full class.

The worked example has signature `(12;01;{0,1})` and belongs to Class 19. The
cell `(1,2)` is inactive and is therefore ignored by the signature.
...
The non-layered underlying patterns `231` and `312` are reported as members of
the zero class, which is not numbered among the 146 nonzero classes.
...
Each exported row records the distribution class, underlying permutation, a
...
nonzero classes, adds the `2*2^16=131072` zero-class diagrams, and checks the
